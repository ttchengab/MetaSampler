...
from .sputils import nn_matching
#
# try:
#     from .soft_projection import SoftProjection
#     from .chamfer_distance import ChamferDistance
...
class ResBlock(nn.Module):
    def __init__(self, input_size, hidden_size=None, output_size=None):
        ...
        self.fc1 = nn.Linear(self.input_size, self.hidden_size, 1)
        self.fc2 = nn.Linear(self.hidden_size, self.output_size, 1)
        self.batchnorm = nn.BatchNorm1d(self.hidden_size)
        self.activation = nn.ReLU()

        self.fc_shortcut = nn.Linear(self.input_size, self.output_size, 1)
        self.shortcut = 0
        if input_size == output_size:
            self.shortcut = 1

    def forward(self, x):
        new_x = self.activation(self.fc1(x))
        new_x = self.fc2(new_x)
        if self.shortcut == 1:
            x_s = self.fc_shortcut(x)
            return self.activation(new_x + x_s)
        return self.activation(new_x + x)
...
class SampleNet(nn.Module):
    def __init__(
        ...
        # Onet Encode query point
        self.fc_point = nn.Linear(point_dim, hidden_dim)
        self.resblock1 = ResBlock(hidden_dim)
        self.resblock2 = ResBlock(hidden_dim)
        self.resblock3 = ResBlock(hidden_dim)
        self.resblock4 = ResBlock(hidden_dim)
        self.resblock5 = ResBlock(hidden_dim)
        self.fc_occupancy = nn.Linear(256, 1)
    ...
    def forward(self, x: torch.Tensor, query_point):
        ...
        y = self.fc3(y)
        point_feature = self.fc_point(query_point)
        occ_x = F.relu(self.bn_fc3(y + point_feature))
        # print(x.shape)
        occ_x = self.resblock1(occ_x)
        occ_x = self.resblock2(occ_x)
        occ_x = self.resblock3(occ_x)
        occ_x = self.resblock4(occ_x)
        occ_x = self.resblock5(occ_x)
        occ_x = nn.Sigmoid()(self.fc_occupancy(occ_x))

        y = F.relu(self.bn_fc3(y))
        y = self.fc4(y)
