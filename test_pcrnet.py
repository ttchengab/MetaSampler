...
def test_one_epoch(device, model, test_loader):
	...
	for i, data in enumerate(tqdm(test_loader)):
		...
		igt_t = igt_t.to(device) - torch.mean(source, dim=1).to(device).unsqueeze(1)
		source = source - torch.mean(source, dim=1, keepdim=True).to(device)
		template = template - torch.mean(template, dim=1, keepdim=True).to(device)
...
def options():
	...
	parser.add_argument('--exp_name', type=str, default='exp_ipcrnet123', metavar='N',
	...
	parser.add_argument('--pretrained', default='checkpoints/exp_ipcrnet3/models/best_model.t7', type=str,
						metavar='PATH', help='path to pretrained model file (default: null (no-use))')
	parser.add_argument('--device', default='cuda:0', type=str,
						metavar='DEVICE', help='use CUDA if available')
# 'pcrnet/pretrained/exp_ipcrnet/models/best_model.t7'
