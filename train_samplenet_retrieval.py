...
def main(args):
    ...
    '''CREATE DIR'''
    timestr = 'retrievalnet' + str(out_points) + '_' + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'))
    exp_dir = Path('./log/')
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (orig_points, diff_points) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            proj_pc = proj_pc.transpose(2, 1)
            one_pred = classifier(proj_pc, points)
            ones = torch.ones(orig_points.shape[0], 1)
            zero_pred = classifier(proj_pc, diff_points)
