...
model = pointnet_cls
checkpoints_dir = ['ptnet_cls_1','ptnet_cls_2','ptnet_cls_3'] # checkpoint names for classification
num_tasks_cls = len(checkpoints_dir)
for i in range(num_tasks_cls):
    classifier = model.get_model(num_class, normal_channel=args.use_normals)
    criterion_cls = model.get_loss()
    checkpoint = torch.load('log/classification/' + checkpoints_dir[i] + '/checkpoints/best_model.pth') # Update checkpoint directory here
    ...
    # task_losses.append(criterion_cls)

# Add Reconstruction Models
checkpoints_dir = ['pcn_1','pcn_2','pcn_3'] #checkpoint names for reconstruction
num_tasks_rec = len(checkpoints_dir)
for i in range(num_tasks_rec):
    pcn_joint = pcn.PCN()
    checkpoint = torch.load('log/samplenet_meta/' + checkpoints_dir[i] + '/best.pth') # Update checkpoint directory here
...
# Add Shape Retrieval Models
checkpoints_dir = ['shape_ret1','shape_ret2','shape_ret3'] #checkpoint names for shape retrieval
num_tasks_ret = len(checkpoints_dir)
for i in range(num_tasks_ret):
    retrievalnet_joint = pointnet_retrieval.get_model(normal_channel=args.use_normals)
    checkpoint = torch.load('log/shape_retrieval/' + checkpoints_dir[i] + '/checkpoints/best_model.pth') # Update checkpoint directory here
...
"""Create Ckpt dir"""
saving_dir = 'log/samplenet_meta_all/'+str(out_points)+'_'+str(num_tasks)+datetime.now().strftime('%Y-%m-%d_%H-%M-%S')+'/'
    ...
    for idx, (points, diff_points, target) in enumerate(tqdm(trainDataLoader)):
        ...
        for i in range(len(task_models)):
            ...
            for j in range(num_steps):
                ...
                if i < num_tasks_cls: #num_tasks_cls should be 3
                    proj_pc = proj_pc.transpose(2, 1)
                    pred, trans_feat = task_models[i](proj_pc)
                    loss = criterion_cls(pred, target.long(), trans_feat)
                elif i < num_tasks_cls + num_tasks_rec: #num_tasks_cls + num_tasks_rec should be 6
        ...
        for i in range(len(task_models)):
            ...
            if i < 3:
                proj_pc = proj_pc.transpose(2, 1)
                pred, trans_feat = task_models[i](proj_pc)
                sum_losses += criterion_cls(pred, target.long(), trans_feat)
            elif i < 6:
                predicted_points = task_models[i](proj_pc)
        ...
        # update sampler (task first followed by simplification and projection loss)
