...
def test(model, loader, num_class=40):
    ...
    for j, (points, target_sets, labels) in tqdm(enumerate(loader), total=len(loader)):
        ...
        for i, target_cloud in enumerate(target_sets):
            ...
            pred_val = classifier(points, target_cloud)
...
def main(args):
    ...
    '''CREATE DIR'''
    timestr = 'retrievalnet_1_' + str(datetime.datetime.now().strftime('%Y-%m-%d_%H-%M'))
    exp_dir = Path('./log/')
    exp_dir.mkdir(exist_ok=True)
    exp_dir = exp_dir.joinpath('shape_retrieval')
    exp_dir.mkdir(exist_ok=True)
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        for batch_id, (orig_points, diff_points) in tqdm(enumerate(trainDataLoader, 0), total=len(trainDataLoader), smoothing=0.9):
            ...
            # print(points.shape)
            one_pred = classifier(orig_points, points)
            ones = torch.ones(orig_points.shape[0], 1)
            zero_pred = classifier(orig_points, diff_points)
        ...
        log_string('Train Instance Accuracy: %f' % loss)

        with torch.no_grad():
            acc = test(classifier.eval(), testDataLoader, num_class=num_class)
